...
class KModLanguageProvider : IModLanguageProvider {
    override fun name() = "liquidstarter"
    ...
    class KotlinModTarget(private val className: String) : IModLanguageProvider.IModLanguageLoader {
        ...
        override fun <T> loadMod(info: IModInfo, modFileScanResults: ModFileScanData, gameLayer: ModuleLayer): T {
            ...
            try {
                val ktContainer = Class.forName("liquid.adapters.KModContainer", true, Thread.currentThread().contextClassLoader)
                LOGGER.debug("Loading KModContainer from classloader ${Thread.currentThread().contextClassLoader} - got ${ktContainer.classLoader}}")
